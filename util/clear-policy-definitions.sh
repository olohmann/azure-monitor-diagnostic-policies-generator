...
for i in `az policy definition list -o tsv --query "[?policyType=='Custom'][].name" --subscription ${SUBSCRIPTION}`; do
    az policy definition delete -n $i --subscription ${SUBSCRIPTION}
    .log 5 "Deleted policy definition $i"
done
