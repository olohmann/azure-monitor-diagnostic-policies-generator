...
for i in `az policy assignment list -o tsv --query "[].name" --subscription ${SUBSCRIPTION}`; do
    az policy assignment delete -n $i --subscription ${SUBSCRIPTION}
    .log 5 "Deleted policy assignment $i"
done
